// SillyTavern 扩展 - 昭华政务生成器 v1.0 (多文件架构)
...
const affairs = [
    {
        ...
        lin: { text: "此人虽狂，但若因言获罪恐失人心。罚他在国子监扫地三日，既惩戒其行，又全了陛下爱才之名。", reply: "林卿思虑周全，便依你所言，罚他去国子-监扫地，磨磨性子。" },
...
class ZhaohuaGovernance {
    ...
    async init() {
        ...
        // 2. 加载HTML和CSS (从外部文件)
        // manifest.json 会自动处理加载，我们只需要绑定事件
        
        // 3. 绑定事件
        // 使用延时确保HTML元素已由ST加载完毕
        setTimeout(() => {
            this.bindEvents();
            console.log("✅ [昭华政务] 初始化成功。");
        }, 500);
    }
